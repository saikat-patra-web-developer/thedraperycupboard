...
export default function ProductDetailPage({ id }) {
  ...
  return <>
    <section className="wrap py-5">
      <nav aria-label="Breadcrumb" className="mb-6 flex flex-wrap gap-3 text-xs text-neutral-500">
      ...
      <div className="grid gap-8 md:grid-cols-[1.4fr_1fr] md:gap-12">
        <div className="min-w-0">
          <div className="relative overflow-hidden rounded-xl">
            ...
            <span className="absolute left-4 top-4 rounded bg-white/95 px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-moss">{product.category}</span>
          </div>
          ...
          <p className="mt-3 text-[10px] text-neutral-500">Illustrative setting. Fabrics, finishes and configurations are selected with your quote.</p>
        </div>
        <div className="py-2">
          ...
          <ul className="my-6 space-y-3">{product.benefits.map(benefit => <li key={benefit} className="flex gap-3 text-sm"><Icon size={18} className="text-moss" />{benefit}</li>)}</ul>
          <div className="rounded-lg bg-[#eeede7] p-4"><p className="text-sm font-semibold">A quote tailored to your space</p><p className="muted mt-1 !text-xs">Share your measurements and preferences for product options and pricing.</p></div>
      ...
      <div className="my-10 grid gap-8 rounded-xl bg-[#f0efeb] p-6 md:grid-cols-3 md:p-8">
      ...
      <div className="grid gap-10 pb-10 md:grid-cols-[1.1fr_1fr]">
        ...
        <div className="rounded-xl border border-neutral-200 bg-white p-6">
          ...
          <dl className="space-y-5">{[["Best suited to",product.bestFor],["Light control",product.light],["Operation",product.movement],["Sizing & availability","Confirmed for your measurements and selected configuration."]].map(([title,text]) => <div key={title}><dt className="text-xs font-semibold text-moss">{title}</dt><dd className="muted mt-1">{text}</dd></div>)}</dl>
        </div>
      </div>
      <div className="grid gap-8 border-t border-neutral-200 py-9 md:grid-cols-[1.6fr_1fr]">
